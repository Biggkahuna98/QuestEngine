...
namespace qrhi
{
    ...
    void InitSlangSession()
    {
        ...
        {
            QE_LOG(Core, Error, "Failed to create Slang global session");
            return;
        }
        ...
        {
            QE_LOG(Core, Error, "Failed to find SPIR-V profile");
            return;
        }
        ...
        {
            QE_LOG(Core, Error, "Failed to create Slang session");
            return;
        }
    ...
    inline void diagnoseIfNeeded(slang::IBlob *diagnosticBlob)
    {
        ...
        {
            QE_LOG(Core, Error, "{}", static_cast<const char*>(diagnosticBlob->getBufferPointer()));
        }
    }
    ...
    void CompileShader(std::string_view shaderName, std::string_view shaderPath)
    {
        ...
        Slang::ComPtr<slang::IModule> slangModule = nullptr;
        {
            ...
            if (!slangModule)
            {
                QE_LOG(Core, Error, "Failed to load module: {}", path);
                return;
            }
        }
        ...
        Slang::ComPtr<slang::IEntryPoint> entryPoint2;
        {
            ...
            if (!entryPoint2)
            {
                QE_LOG(Core, Error, "Failed to find entry point: fragMain");
                return;
            }
        }
        ...
        Slang::ComPtr<slang::IComponentType> composedProgram;
        {
            ...
            {
                QE_LOG(Core, Error, "Failed to create composite component type: {}", path);
                return;
            }
        }
        ...
        Slang::ComPtr<slang::IComponentType> program;
        {
            ...
            if (SLANG_FAILED(result))
            {
                QE_LOG(Core, Error, "Failed to link component type: {}", path);
                return;
            }
        }
        ...
        Slang::ComPtr<ISlangBlob> spirv;
        {
            ...
            if (SLANG_FAILED(result))
            {
                QE_LOG(Core, Error, "Failed to get entry point code: {}", path);
                return;
            }
        }
        ...
        if (byteSize % 4 != 0)
        {
            QE_LOG(Core, Error, "SPIR-V bytecode is not a multiple of 4 bytes: {}", path);
            return;
        }
        ...
        if (!file.is_open())
        {
            QE_LOG(Core, Error, "Failed to open shader cache file: {}", cacheName);
            return;
        }
    ...
    std::vector<uint32_t> ReadShaderCache(std::string_view shaderName)
    {
        ...
        if (!file.is_open())
        {
            QE_LOG(Core, Error, "Failed to open shader cache file: {}", cachePath);
            return {};
        }
        ...
        if (size % 4 != 0)
        {
            QE_LOG(Core, Error, "Shader cache file is not a multiple of 4 bytes: {}", cachePath);
        }
