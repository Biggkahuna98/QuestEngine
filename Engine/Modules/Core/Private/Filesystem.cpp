...
namespace Quest::Utils
{
    ...
    std::string LoadShaderFromFile(const std::string &shaderName)
    {
        ...
        if (!file.is_open())
        {
            QE_LOG(Core, Error, "Failed to open shader file: {}", shaderName);
            return "";
        }
    ...
    void WriteSPIRVToCache(const std::string &shaderName, const std::vector<uint32_t> &spirv)
    {
        ...
        if (!file.is_open())
        {
            QE_LOG(Core, Error, "Failed to open shader cache file: {}", ConvertShaderNameToCacheName(shaderName));
            return;
        }
    ...
    std::vector<uint32_t> LoadSPIRVFromCache(const std::string &shaderName)
    {
        ...
        if (size % 4 != 0)
        {
            QE_LOG(Core, Error, "Shader cache file is not a multiple of 4 bytes: {}", ConvertShaderNameToCacheName(shaderName));
            return {};
        }
        ...
        {
            QE_LOG(Core, Error, "Failed to read shader cache file: {}", ConvertShaderNameToCacheName(shaderName));
            return {};
        }
        file.close();

        if (buffer[0] != 0x07230203)
            QE_LOG(Core, Error, "Invalid SPIR-V magic number: 0x{:08x}", buffer[0]);
