...
namespace Quest
{
    ...
    protected:
        // Out-of-line key function: anchors the vtable/typeinfo in Core.dll so the exported
        // RefCounted base resolves for consumers in other modules (RHI, Render).
        //
        // Protected, not public: intrusively-refcounted objects must be owned through
        // RefCountPtr (which calls Release()); external code must not `delete` them or hold
        // them by value/unique_ptr. The eventual `delete this` belongs inside Release(), a member.
        virtual ~RefCounted();
    private:
